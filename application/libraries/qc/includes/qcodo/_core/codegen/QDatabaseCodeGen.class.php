...
//								$objReferencedTable = $this->objTableArray[strtolower($objReference->Table)];
		...
		protected function AnalyzeTableColumn(QDatabaseFieldBase $objField, $objTable) {
			...
			// original
			// if (($objField->PrimaryKey) && $objTable && (!empty($objTable->PrimaryKeyColumnArray)) && (count($objTable->PrimaryKeyColumnArray) == 1)) {
			//	$objColumn->Unique = true;
			// }

			// moch_isnaini # 2024-07-08
			// echo $objField->Name."\n";
			// echo $objField->PrimaryKey."\n";
			// var_dump($objTable);
			// echo "\n";
			// original
			if (($objField->PrimaryKey) && $objTable) {
				$objColumn->Unique = true;
			}
			if ((!empty($objTable->PrimaryKeyColumnArray)) && (count($objTable->PrimaryKeyColumnArray) == 1)) {
				$objColumn->Unique = true;
			}
			// echo $objField->Name."\n";
			// echo $objColumn->Unique."\n";
			// moch_isnaini # 2024-07-08
